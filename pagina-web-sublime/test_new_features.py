...
# Configurar rutas y base de datos temporal con NullPool para evitar caching
app_module.SHARED_DB_PATH = _TEMP_DB
app_module.SHARED_SQL_PATH = _ORIG_SQL
app.config["SQLALCHEMY_ENGINE_OPTIONS"] = {"poolclass": NullPool}
app.config["SQLALCHEMY_DATABASE_URI"] = f"sqlite:///{_TEMP_DB}"
app.config["TESTING"] = True
app.config["SECRET_KEY"] = "test_secret"

# Re-registrar SQLAlchemy para forzar la recreación de los motores con NullPool
try:
    from models import db
    app.extensions.pop("sqlalchemy", None)
    db.init_app(app)
except Exception as e:
    print("RE-REGISTRATION FAILED:", e)

def _clear_sqlalchemy_engines():
    try:
        from models import db
...
# No inicializar ni abrir conexiones a la base de datos temporal aquí.
# Dejaremos que _reset_db() se encargue de crear la BD por primera vez.
...
def _reset_db():
    ...
    # Crear tablas de SQLAlchemy en la BD temporal recién creada
    try:
        from models import db
        with app.app_context():
            db.create_all()
    except Exception as e:
        print("Failed to run db.create_all():", e)
